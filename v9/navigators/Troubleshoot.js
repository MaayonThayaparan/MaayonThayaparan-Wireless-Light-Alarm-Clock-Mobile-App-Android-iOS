...
const Troubleshoot = ({ navigation }) => {
  ...
  const [apiKey, setApiKey] = useState(userInfo.apiKey === GOVEE_API_KEY ? '' : userInfo.apiKey);
  const [urlGet, setUrlGet] = useState(userInfo.urlGet === GOVEE_URL_GET ? '' : userInfo.urlGet);
  const [urlPost, setUrlPost] = useState(userInfo.urlPost === GOVEE_URL_POST ? '' : userInfo.urlPost);
